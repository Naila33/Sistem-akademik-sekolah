...
<div class="card">
    ...
    <form action="{{ route('guru.store') }}" method="POST">@csrf
        ...
        <p><label>Agama<br><select name="agama" required>
                    <option value="">Pilih</option>@foreach(['Islam','Kristen','Katolik','Hindu','Budha','Konghucu'] as $agama)<option value="{{ $agama }}">{{ $agama }}</option>@endforeach
                </select></label></p>
        ...
        <p><label>Status Kepegawaian<br><select name="status_kepegawaian" required>
                    <option value="">Pilih</option>@foreach(['PNS','PPPK','Honorer','Guru_tetap','Guru_tidak_tetap'] as $status)<option value="{{ $status }}">{{ str_replace('_',' ',$status) }}</option>@endforeach
                </select></label></p>
        <p><label>Jabatan<br><select name="jabatan" required>
                    <option value="">Pilih</option>@foreach(['Guru','Kepala_sekolah','Waka_sekolah'] as $jabatan)<option value="{{ $jabatan }}">{{ str_replace('_',' ',$jabatan) }}</option>@endforeach
                </select></label></p>
        ...
        <p><label>Mata Pelajaran<br><select name="mata_pelajaran_id" required>
                    <option value="">Pilih</option>@foreach($mataPelajaran as $mapel)<option value="{{ $mapel->id }}">{{ $mapel->nama_mapel }}</option>@endforeach
                </select></label></p>
