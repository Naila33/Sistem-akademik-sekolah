...
@section('content')
<h1>Tambah Ruangan</h1>@if($errors->any())<ul>@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul>@endif
<form action="{{ route('ruangan.store') }}" method="POST">@csrf
    ...
    <p><label>Kapasitas<br><input type="number" name="kapasitas" value="{{ old('kapasitas') }}" min="1" required></label></p>
